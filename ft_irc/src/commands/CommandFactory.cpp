...
Command* CommandFactory::createCommand(const std::string& commandName, Server* server)
{
    ...
    std::map<std::string, CommandCreator>::iterator it = _commandCreators.find(upperCommandName);
...
void CommandFactory::registerCommand(const std::string& commandName, CommandCreator creator)
{
